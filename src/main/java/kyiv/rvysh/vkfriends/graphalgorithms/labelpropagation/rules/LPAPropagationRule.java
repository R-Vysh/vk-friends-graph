 ...
 */
public class LPAPropagationRule<T> extends PropagationRuleBase<T> {

	public LPAPropagationRule(LabelPropagationClusterer<T> clusterer) {
		super(clusterer);
	}
	 ...
	 * rule selects a random one from the dominant group provided that the
	 * processed node is not already bearing a dominant one. In such a case, the
	 * rule keeps the current label of the processed node.
	 */
	@Override
	public Integer getDominantClusterInNeighbourhood(T node) {
		...
		if (neighbours.isEmpty()) {
			...
			// no clusters in neighborhood, a disconnected node.
		}
		...
		Integer prevailingCluster = MapUtils.getKeyByValue(neighborClusterWeights, maxWeight);
		...
		if (neighborClusterWeights.containsKey(currentNodeCluster)) {
			...
			if (neighborClusterWeights.get(currentNodeCluster) == maxWeight)
				prevailingCluster = currentNodeCluster;
		}
		return prevailingCluster;
	}
}
