...
public class ICloseness<V, E> extends GraphProximity<V,E> {
	...
	//TODO: normalize it !!!
	public ICloseness(Graph<V, E> g) {
		this(g, null);
	}
	
	public ICloseness(Graph<V, E> g, Transformer<E, ? extends Number> weight) {	
		this( 2, g, weight);
	}
	
	public ICloseness(int neighborhoodThreshold, Graph<V, E> g, Transformer<E, ? extends Number> weight) {
		super(g, weight);		
...
//		union.add(v1);union.add(v2);
		...
		for (V v: union) {
			//if(Nv2.get(v)!=null){
				res += ns(Nv1,v)*ns(Nv2,v);
				dom += ns(Nv1,v)*ns(Nv1,v) + ns(Nv2,v)*ns(Nv2,v);
			//}	
		}
//		if(graph.getNeighbors(v1).contains(v2)){
			res +=  ns(Nv1,v2)*ns(Nv2,v1);
			dom += ns(Nv1,v2)*ns(Nv1,v2) + ns(Nv2,v1)*ns(Nv2,v1);
//		}
		
//		System.err.println(res + " " + dom );
		return (res / (dom - res));
	}
	...
	public String toString(){
		return "iCloseness"+type+neighborhoodThreshold;
	}
	@Override
	public String getName() {
		return "IC"+neighborhoodThreshold;
	}
