...
public class AdjacencyPearsonCorrelation<V, E> extends GraphProximity<V, E> {
	...
	public AdjacencyPearsonCorrelation(Graph<V, E> graph, Transformer<E, ? extends Number> weights, boolean selfWeight,
			boolean normalize) {
		this(graph, weights, selfWeight, false, normalize);
	}

	public AdjacencyPearsonCorrelation(Graph<V, E> graph, Transformer<E, ? extends Number> weights, boolean selfWeight,
			boolean normalizedWeight, boolean normalize) {
	...
	public Number computeMeasure(V source, V target) {
		...
		double sk, tk, n = graph.getVertexCount() + (selfWeight ? 1 : 0), sumST = 0, sumS = 0, sumT = 0, sum2S = 0,
				sum2T = 0;
	...
	// }
	// //WTF??? m+= m*m *
	// (graph.getEdgeCount()-graph.getIncidentEdges(v).size());
